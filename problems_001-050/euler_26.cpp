...
unsigned int length_of_periodic_part(
    unsigned int m, 
    unsigned int n, 
    bool verbose=false)
{
  ...
  while (m != 0) {
    ...
    if (m != 0 && std::find(vec.begin(), vec.end(), m) != vec.end()) { 
      length = std::distance(std::find(vec.begin(), vec.end(), m), vec.end());

      if (verbose) {
        std::cout << "Length of the periodic part: "<< length << std::endl;
        std::cout << "Decimals: .";
        std::vector<unsigned int>::iterator it;
        for (it = decimals.begin(); it != decimals.end(); it++)
          std::cout << *it;
        std::cout << std::endl;
      }
      break; 
...
{
  // // Test on te example:
  // std::cout << "1/2 has the cycle length: " 
  //           << length_of_periodic_part(1, 2, true) << std::endl;
  // std::cout << "1/6 has the cycle length: " 
  //           << length_of_periodic_part(1, 6, true) << std::endl;
  // std::cout << "1/7 has the cycle length: " 
  //           << length_of_periodic_part(1, 7, true) << std::endl;
  // std::cout << "1/17 has the cycle length: " 
  //           << length_of_periodic_part(1, 17, true) << std::endl;
